    ...
    IMapper mapper, IHubContext<PresenceHub> presenceHub) : Hub
{
    ...
    public async Task SendMessage(CreateMessageDto createMessageDto)
    {
        ...
        if (username.Equals(createMessageDto.RecipientUsername, StringComparison.InvariantCultureIgnoreCase))
            throw new HubException("You cannot send messages to yourself");

        var sender = await unitOfWork.UserRepository.GetUserByUsernameAsync(username);
        var recipient = await unitOfWork.UserRepository.GetUserByUsernameAsync(createMessageDto.RecipientUsername);
        if (recipient == null || sender == null || sender.UserName == null || recipient.UserName == null)
            throw new HubException("Cannot sed message at this time");
    ...
    private async Task<Group> AddToGroup(string groupName)
    {
        ...
        throw new HubException("Cannot add to group");
    }
    ...
    public async Task<Group> RemoveFromMessageGroup()
    {
        ...
        throw new HubException("Cannot remove from group");
    }

    private static string GetGroupName(string username, string? otherUsername)
    {
